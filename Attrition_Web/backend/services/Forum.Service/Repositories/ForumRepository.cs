...
public class ForumRepository(ForumDbContext context) : Repository<ForumPost>(context), IForumRepository
{
    ...
    {
        var query = from p in context.ForumPosts join root in context.ForumPosts on p.RootPostId equals root.Id
                    where p.AuthorId == userId && !p.Moderation.IsRemoved && root.WikiArticleId == null
                    orderby p.CreatedAt descending select new UserReplyRow(p.Id, root.Id, root.Title ?? "Discussion", p.Content, p.CreatedAt);

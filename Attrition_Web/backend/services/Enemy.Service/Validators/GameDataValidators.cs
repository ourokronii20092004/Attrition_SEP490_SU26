...
public class GameDataImportRequestValidator : AbstractValidator<GameDataImportRequest>
{
    ...
    private static void AddDuplicates(IEnumerable<string> ids,string property,ValidationContext<GameDataImportRequest> context) { foreach(var id in ids.GroupBy(x=>x,StringComparer.Ordinal).Where(x=>x.Count()>1).Select(x=>x.Key)) context.AddFailure(property,$"Duplicate stable ID '{id}'."); }
}

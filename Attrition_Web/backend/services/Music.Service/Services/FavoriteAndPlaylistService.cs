...
public class FavoriteService : IFavoriteService
{
    ...
    public FavoriteService(IMusicRepository repository) => _repository = repository;

    public async Task<IEnumerable<FavoriteTrackDto>> GetFavoritesAsync(Guid userId)
    {
        var favorites = await _repository.Favorites.ListAsync(f => f.UserId == userId,
            q => q.OrderByDescending(f => f.AddedAt));

        var trackIds = favorites.Select(f => f.TrackId).Distinct().ToList();
        var tracks = (await _repository.Tracks.ListAsync(t => trackIds.Contains(t.TrackId)))
            .ToDictionary(t => t.TrackId);
        var albumIds = tracks.Values.Select(t => t.AlbumId).Distinct().ToList();
        var albums = (await _repository.Albums.ListAsync(a => albumIds.Contains(a.AlbumId)))
    ...
    {
        var (existingList, _) = await _repository.Favorites.GetPagedAsync(1, 1, f => f.UserId == userId && f.TrackId == trackId);
        var existing = existingList.FirstOrDefault();
        if (existing != null)
        {
            await _repository.Favorites.DeleteAsync(existing);
            return (true, false, null);
        }

        var track = await _repository.Tracks.GetByIdAsync(trackId);
        if (track == null) return (false, false, "Track not found");

        await _repository.Favorites.AddAsync(new UserFavorite { UserId = userId, TrackId = trackId });
...
public class PlaylistService : IPlaylistService
{
    ...
    public PlaylistService(IMusicRepository repository) => _repository = repository;

    public async Task<IEnumerable<PlaylistDto>> GetPlaylistsAsync(Guid userId)
    {
        var playlists = await _repository.Playlists.ListAsync(p => p.UserId == userId, q => q.OrderByDescending(p => p.UpdatedAt));
        return playlists.Select(ToDto);
    }

    public Task<MusicPlaylist?> GetPlaylistAsync(Guid id) => _repository.Playlists.GetByIdAsync(id);
    ...
    {
        var playlist = await _repository.Playlists.GetByIdAsync(playlistId);
        ...
        var links = await _repository.PlaylistTracks.ListAsync(pt => pt.PlaylistId == playlistId,
            q => q.OrderBy(pt => pt.Position).ThenBy(pt => pt.AddedAt));
        var trackIds = links.Select(l => l.TrackId).ToList();
        var tracks = (await _repository.Tracks.ListAsync(t => trackIds.Contains(t.TrackId))).ToDictionary(t => t.TrackId);
        var albumIds = tracks.Values.Select(t => t.AlbumId).Distinct().ToList();
        var albums = (await _repository.Albums.ListAsync(a => albumIds.Contains(a.AlbumId))).ToDictionary(a => a.AlbumId);
    ...
    public async Task<PlaylistDto> CreatePlaylistAsync(Guid userId, string name, string? description)
    {
        ...
        var created = await _repository.Playlists.AddAsync(playlist);
    ...
    public async Task<PlaylistOpResult> AddTrackToPlaylistAsync(Guid userId, Guid playlistId, int trackId)
    {
        ...
        if (await _repository.Tracks.GetByIdAsync(trackId) == null) return PlaylistOpResult.NotFound;

        var (existing, _) = await _repository.PlaylistTracks.GetPagedAsync(1, 1, pt => pt.PlaylistId == playlistId && pt.TrackId == trackId);
        if (existing.Count > 0) return PlaylistOpResult.Ok;

        // Append after the current last track (positions are 0-based).
        var current = await _repository.PlaylistTracks.ListAsync(pt => pt.PlaylistId == playlistId);
        var nextPos = current.Select(pt => pt.Position).DefaultIfEmpty(-1).Max() + 1;
        await _repository.PlaylistTracks.AddAsync(new PlaylistTrack { PlaylistId = playlistId, TrackId = trackId, Position = nextPos });
    ...
    {
        playlist.TrackCount = await _repository.PlaylistTracks.CountAsync(pt => pt.PlaylistId == playlistId);
        playlist.UpdatedAt = DateTime.UtcNow;
        await _repository.Playlists.UpdateAsync(playlist);
    }
    ...
    public async Task<PlaylistOpResult> RemoveTrackFromPlaylistAsync(Guid userId, Guid playlistId, int trackId)
    {
        ...
        await _repository.PlaylistTracks.DeleteAsync(pt);
        await SyncTrackCountAsync(playlist, playlistId);
        return PlaylistOpResult.Ok;
    }
    ...
    public async Task<PlaylistOpResult> ReorderPlaylistAsync(Guid userId, Guid playlistId, IReadOnlyList<int> trackIds)
    {
        ...
        var links = (await _repository.PlaylistTracks.ListAsync(pt => pt.PlaylistId == playlistId)).ToList();
        ...
        foreach (var trackId in trackIds)
        {
            ...
            {
                link.Position = pos++;
                await _repository.PlaylistTracks.UpdateAsync(link);
            }
        }
    ...
    public async Task<PlaylistOpResult> DeletePlaylistAsync(Guid userId, Guid playlistId)
    {
        ...
        await _repository.Playlists.DeleteAsync(playlist);
        return PlaylistOpResult.Ok;
    }
}
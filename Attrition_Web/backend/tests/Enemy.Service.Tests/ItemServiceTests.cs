...
public class ItemServiceTests
{
    ...
    public async Task Browse_ForwardsFiltersAndMapsRows(string? category, string? search, string _)
    {
        ...
        await _repo.Received(1).GetAllWithModifiersAsync(category, search);
        if (result.Count > 0)
        {
            Assert.Equal(2, result[0].Modifiers.Count);
        }
    }
    ...
    {
        ItemEntity? inserted = null;
        _repo.TryAddAsync(Arg.Do<ItemEntity>(x => inserted = x)).Returns(true);
        var result = await Sut.CreateAsync(Create());
        Assert.True(result.Success);
        Assert.Equal("steel_helm", inserted!.ItemId);
        Assert.Contains("item-list:", _cache.Removed);
        Assert.Contains("item-bundle:all", _cache.Removed);
    }
    ...
    public async Task Add_UTCID05_NullDescription_IsPreserved()
    {
        ItemEntity? inserted = null;
        _repo.TryAddAsync(Arg.Do<ItemEntity>(x => inserted = x)).Returns(true);
        await Sut.CreateAsync(Create(description: null));
    ...
    public async Task Add_UTCID06_Modifiers_AreMapped()
    {
        ...
        Assert.Equal(2, inserted!.Modifiers.Count);
        Assert.Equal(5, inserted.Modifiers[0].Amount);
    }

    [Fact]
    public async Task Edit_UTCID01_NullModifiers_KeepsExistingList()
    {
        await AssertEdit(null, 2);
    }

    [Fact]
    public async Task Edit_UTCID02_NewModifierList_ReplacesExisting()
    {
        await AssertEdit(new() { new("AD", 9) }, 1);
    }

    [Fact]
    public async Task Edit_UTCID03_EmptyModifierList_ClearsExisting()
    {
    ...
    {
        var item = Item();
        _repo.GetWithModifiersAsync(item.ItemId).Returns(item);
        var result = await Sut.UpdateAsync(item.ItemId, Update(mods: mods));
        Assert.True(result.Success);
        Assert.Equal(expected, item.Modifiers.Count);
    ...
    {
        var item = Item(id);
        _repo.GetByIdAsync(id).Returns(item);
        Assert.True((await Sut.DeleteAsync(id)).Success);
        await _repo.Received(1).DeleteAsync(item);
        Assert.Contains("item-list:", _cache.Removed);
    }

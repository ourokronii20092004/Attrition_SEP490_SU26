...
public class SearchServiceTests
{
    ...
    {
        var sut = Create(); var r = await sut.GlobalSearchAsync("dragon", 10, true, default);
        Assert.Single(r.Wiki); Assert.Single(r.Users); Assert.Single(r.Posts); Assert.Single(r.Enemies); Assert.Single(r.Items); Assert.Single(r.Skills); Assert.Empty(r.DegradedSources);
    }

    [Fact]
    public async Task UTCID02_EmptyQuery_ReturnsEmptyWithoutDownstreams()
    {
        var h = new Handler(); var r = await Create(h).GlobalSearchAsync("   ", 10, true, default); Assert.Empty(r.Wiki); Assert.Equal(0, h.Calls);
    }

    [Fact]
    public async Task UTCID03_WikiScope_CallsOnlyWiki()
    {
        var h = new Handler(); var r = await Create(h).GlobalSearchAsync("wiki: flame", 10, true, default); Assert.Single(r.Wiki); Assert.Equal(1, h.Calls);
    }

    [Fact]
    public async Task UTCID04_IncludeUsersFalse_OmitsIdentity()
    {
        var h = new Handler(); var r = await Create(h).GlobalSearchAsync("dragon", 10, false, default); Assert.Empty(r.Users); Assert.Equal(5, h.Calls);
    }

    [Fact]
    public async Task UTCID05_DownstreamFailure_DegradesOnlyThatSource()
    {
        var h = new Handler("forum"); var r = await Create(h).GlobalSearchAsync("dragon", 10, true, default); Assert.Empty(r.Posts); Assert.Contains("forum", r.DegradedSources); Assert.Single(r.Wiki);
    }

    [Fact]
    public async Task UTCID06_CachedResponse_SkipsDownstreams()
    {
        var h = new Handler(); var c = new Cache(); var expected = GlobalSearchResponse.Empty(); c.Value = expected; Assert.Same(expected, await Create(h, c).GlobalSearchAsync("dragon", 10, true, default)); Assert.Equal(0, h.Calls);
    }

    [Fact]
    public async Task UTCID07_DegradedResponse_IsNotCached()
    {
        var c = new Cache(); await Create(new Handler("wiki"), c).GlobalSearchAsync("dragon", 10, true, default); Assert.Equal(0, c.Sets);
    }

    [Fact]
    public async Task UTCID08_HealthyResponse_IsCachedForReuse()
    {
        var c = new Cache(); await Create(new Handler(), c).GlobalSearchAsync("dragon", 10, true, default); Assert.Equal(1, c.Sets);
    }

    private static SearchService Create(Handler? handler = null, Cache? cache = null)
    {
        handler ??= new(); cache ??= new(); var config = new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?> { { "Internal:ApiKey", "test" } }).Build();
        HttpClient Client() => new(handler, false) { BaseAddress = new Uri("http://test/") };

...
export function AudioPlayer() {
  const {
    currentTrack, isPlaying, shuffle, repeat,
    next, prev, stop, toggleShuffle, cycleRepeat,
  } = useAudioStore();

  const {
    audioRef, muted, setMuted, displayTime, pct, duration, volume,
    ...
    <>
      ...
      {currentTrack && expanded && (
        <NowPlaying
          ...
          shuffle={shuffle} repeat={repeat} volume={muted ? 0 : volume} muted={muted} liked={liked} canFavorite={canFavorite}

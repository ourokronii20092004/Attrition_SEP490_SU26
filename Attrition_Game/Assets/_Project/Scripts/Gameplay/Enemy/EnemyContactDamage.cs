...
public class EnemyContactDamage : MonoBehaviour
{
    ...
    [Tooltip("Có EnemyStats ở parent → đây là HỆ SỐ × AD. Không → số raw cũ.")]
    ...
    private Attrition.Gameplay.Enemy.EnemyStats parentStats;
    private float lastContactTime = -999f;

    private void Awake()
    {
        parentDamageable = GetComponentInParent<IDamageable>();
        parentStats = GetComponentInParent<Attrition.Gameplay.Enemy.EnemyStats>();
    }
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        ...
        int raw = parentStats != null ? contactDamage * parentStats.AD : contactDamage;
        damageable.TakeDamage(raw, knockbackDir, contactKnockbackForce);
